...
export function deniedPage(email) {
  ...
  // Open Gmail's web compose (not the OS default mail app, which is Outlook on
  // Windows). authuser pins the sender to the account the visitor just signed in
  // with; to / subject / body are unchanged.
  const gmailCompose =
    `https://mail.google.com/mail/?view=cm&fs=1&tf=1` +
    `&to=${encodeURIComponent(ACCESS_REQUEST_CONTACT)}` +
...
<body>
  <div class="card">
    ...
    <a class="btn" href="${gmailCompose}" target="_blank" rel="noopener">Request access</a>
